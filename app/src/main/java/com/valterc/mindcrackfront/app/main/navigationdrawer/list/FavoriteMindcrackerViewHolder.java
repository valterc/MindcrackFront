package com.valterc.mindcrackfront.app.main.navigationdrawer.list;

import android.widget.ImageView;
import android.widget.LinearLayout;
import android.widget.RelativeLayout;
import android.widget.TextView;

import com.vcutils.views.PixelImageView;

/**
 * Created by Valter on 18/05/2014.
 */
public class FavoriteMindcrackerViewHolder {

    public RelativeLayout relativeLayoutBack;
    public PixelImageView imageViewLogo;
    public TextView textViewName;
    public LinearLayout linearLayoutUnseenVideos;
    public TextView textViewUnseenVideosCount;

}
